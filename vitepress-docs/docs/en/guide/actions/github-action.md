...
- Worker backend `secrets`
   ...
   | Name                           | Description                                                                                                                                    |
   | ------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------- |
   | `BACKEND_TOML`                 | Backend configuration file, [see here](/en/guide/cli/worker.html#modify-wrangler-toml-configuration-file). If you deploy features like Turnstile or R2/S3 attachments via GitHub Actions, it is recommended to store those Worker variables directly in `BACKEND_TOML` and update the repository secret instead of only changing them temporarily in the Cloudflare dashboard |
