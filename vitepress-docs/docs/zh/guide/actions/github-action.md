...
- worker 后端 `secrets`
   ...
   | 名称                           | 说明                                                                                                                                    |
   | ------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------- |
   | `BACKEND_TOML`                 | 后端配置文件，[参考此处](/zh/guide/cli/worker.html#修改-wrangler-toml-配置文件)。如果使用 GitHub Actions 部署 Turnstile、R2/S3 附件等 Worker 变量，建议直接把对应配置写入 `BACKEND_TOML` 并更新仓库 secret，而不是只在 Cloudflare 控制台临时修改 |
